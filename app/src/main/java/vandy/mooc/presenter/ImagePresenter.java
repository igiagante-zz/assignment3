    ...
    public void startProcessing() {
        if (mUrlList.isEmpty())
            ...
            for (Uri uri : mUrlList) {
                ImageFilterAsyncTask taskFilter = new ImageFilterAsyncTask(getActivityContext(), this);
                ImageDownloadAsyncTask task = new ImageDownloadAsyncTask(getActivityContext(), taskFilter, mDirectoryPathname);
                task.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, uri.toString(), mDirectoryPathname.toString());
            }
        }
    }
